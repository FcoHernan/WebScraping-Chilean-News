		...
		if ( have_posts() ):
			...
			while ( have_posts() ) : the_post();
				...
				<div <?php post_class( array( 'entry-box', 'clearfix' ) ); ?>>
					<header>
						<h1 class="entry-title" itemprop="name"><?php echo wp_kses_post( $post_title ); ?></h1>

						<?php if ( 'true' == $is_display_post_datetime ): ?>
							<span class="entry-date clearfix"><?php echo KopaIcon::getIcon( 'fa fa-calendar-o entry-icon', 'span' ); ?><span  class="date updated" itemprop="datePublished" content="<?php get_the_date( 'F S, Y' ); ?>"><?php echo get_the_date(); ?></span></span>
						<?php endif; ?>

						<?php if ( 'true' == $is_display_post_views ): ?>
							<span class="entry-views clearfix"><?php echo KopaIcon::getIcon( 'fa fa-eye entry-icon', 'span' ); ?><a href="<?php echo esc_url( $post_url ); ?>" class="kopa-total-views-for-singular"><?php echo KopaUtil::get_views( $post_id ); ?></a></span>                                
						<?php endif; ?>
							...
							<footer class="clearfix">
								<?php if ( isset( $prev_and_next['prev'] ) ): ?>
									<p class="prev-post pull-left clearfix">                        
										<a class="clearfix" href="<?php echo esc_url( $prev_and_next['prev']['url'] ); ?>"><?php echo KopaIcon::getIcon( 'fa fa-angle-left', 'span' ); ?>&nbsp;<?php _e( 'Previous article', kopa_get_domain() ); ?></a>                            
										<a href="<?php echo esc_url( $prev_and_next['prev']['url'] ); ?>" class="article-title"><?php echo wp_kses_post( $prev_and_next['prev']['title'] ); ?></a>
									</p>
								<?php endif; ?>

								<?php if ( isset( $prev_and_next['next'] ) ): ?>
									<p class="next-post pull-right clearfix">
										<a class="clearfix" href="<?php echo esc_url( $prev_and_next['next']['url'] ); ?>"><?php _e( 'Next article', kopa_get_domain() ); ?>&nbsp;<?php echo KopaIcon::getIcon( 'fa fa-angle-right', 'span' ); ?></a>
										<a href="<?php echo esc_url( $prev_and_next['next']['url'] ); ?>" class="article-title"><?php echo wp_kses_post( $prev_and_next['next']['title'] ); ?></a>
									</p>
								<?php endif; ?>
							</footer>
						<?php endif; ?>

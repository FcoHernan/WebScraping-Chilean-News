            ...
            if ($posts->have_posts()) {
                ...
                while ($posts->have_posts()) {
                    ...
                    if ('true' != $instance['is_hide_created_date']) {
                        $shortcode.= sprintf('<span class="entry-date clearfix">%s<span class="date updated">%s</span></span>', KopaIcon::getIcon('fa fa-calendar-o entry-icon', 'span'), get_the_date());
                    }

                    if ('true' != $instance['is_hide_comments']) {
                        $shortcode.= sprintf('<span class="entry-comments clearfix">%s<a href="%s">%s</a></span>', KopaIcon::getIcon('fa fa-comment-o entry-icon', 'span'), get_comments_link($post_id), KopaUtil::get_comments($post_id));
                    }

                    if ('true' != $instance['is_hide_views']) {
                        $shortcode.= sprintf('<span class="entry-views clearfix">%s<a href="%s">%s</a></span>', KopaIcon::getIcon('fa fa-eye entry-icon', 'span'), $post_url, KopaUtil::get_views($post_id));
                    }
                    ...
                    if ('true' != $instance['is_hide_readmore']) {
                        $shortcode.= sprintf('<a class="more-link clearfix" href="%s">%s<span>%s</span></a>', $post_url, KopaIcon::getIcon('fa fa-link entry-icon', 'span') ,__('Read more ...', kopa_get_domain()));
                    }

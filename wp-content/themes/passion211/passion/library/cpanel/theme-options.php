					...
					<div class="<?php echo implode( ' ', $tab_classes ); ?>" id="<?php printf( 'tab-%s', $slug ); ?>">
						...
						foreach ( $tab['groups'] as $groups ) {
							...
							<div id="<?php printf( 'kopa-group-%s', KopaUtil::str_uglify( $groups['title'] ) ); ?>">                        
								<p class="kopa-tab-title clearfix"><span class="kopa-tab-title-left pull-left"><?php echo wp_kses_post( $groups['title'] ); ?></span><span onclick="KopaThemeOptions.onClickSubTab(event, jQuery(this));" class="kopa_subtab_action pull-right dashicons <?php echo esc_attr( $sub_tab_icon ); ?>"></span></p>
								<div class="kopa-tab-body" style="display: <?php echo esc_attr( $sub_tab_display ); ?>;">
									<?php

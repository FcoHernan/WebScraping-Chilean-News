...
function kopa_add_home_menuitem( $items, $args ) {
	if ( 'primary-nav' == $args->theme_location && 'main-menu' == $args->menu_id ) {
		$home_html = sprintf( '<li class="home-menu-icon"><a class="fa fa-home" href="%s" title="%s"></a></li>', home_url(), __( 'Home', kopa_get_domain() ) );
...
function kopa_head() {
	...
	if ( 'true' == KopaOptions::get_option( 'seo_status', 'false' ) ) {
		...
		$current_lang = defined( 'WPLANG' ) ? constant( 'WPLANG' ) : 'en_US';
		printf( '<meta property="og:locale" content="%s">', $current_lang );
...
function kopa_front_enqueue_scripts() {
	if ( !is_admin() ) {
		...
		wp_enqueue_style( KOPA_OPT_PREFIX . 'font-awesome', "{$dir}/library/assets/font-awesome/css/font-awesome.css" );
		...
		wp_enqueue_style( KOPA_OPT_PREFIX . 'magnific-popup', $dir . '/css/jquery.magnific-popup.css', array(), NULL );
		...
		wp_enqueue_script( KOPA_OPT_PREFIX . 'retinma', $dir . '/assets/fotorama/fotorama.js', array( 'jquery' ), NULL, TRUE );
		wp_enqueue_script( KOPA_OPT_PREFIX . 'flickra', $dir . '/js/retina.js', array( 'jquery' ), NULL, TRUE );
		...
		wp_enqueue_script( KOPA_OPT_PREFIX . 'magnific-popup', $dir . '/js/jquery.magnific-popup.js', array( 'jquery' ), NULL, TRUE );
		wp_enqueue_script( KOPA_OPT_PREFIX . 'fotora', $dir . '/js/jquery.flickr.js', array( 'jquery' ), NULL, TRUE );
		...
		if ( is_page() ) {
			...
			wp_enqueue_script( KOPA_OPT_PREFIX . 'gmaps', $dir . '/js/gmaps.js', array( 'jquery', KOPA_OPT_PREFIX . 'maps-api' ), NULL, TRUE );
		}
	}
}
	...
	if ( $limit ) {
		...
		if ( $posts->have_posts() ) {
			...
			<div class="kp-headline-wrapper pull-left clearfix">
				<?php if ( $prefix ): ?>
					<span class="kp-headline-title"><?php echo wp_kses_post( $prefix ); ?></span>
				<?php endif; ?>
				<div class="kp-headline clearfix">                        
					<dl class="ticker-1 clearfix">
						...
						while ( $posts->have_posts() ) {
							...
							?>
							<dd><a href="<?php echo esc_url( $post_url ); ?>"><span  class="date updated"><?php echo get_the_date(); ?></span> - <?php echo wp_kses_post( $post_title ); ?></a></dd>                            
							<?php
						}
						?>                       
					</dl>                    
...
function kopa_get_related_posts() {
	if ( is_single() ) {
		$get_by = KopaOptions::get_option( 'post_related_posts_get_by', 'post_tag' );
									...
									<li <?php post_class( 'masonry-box' ); ?>>
										<article class="entry-item clearfix">
											...
											if ( has_post_thumbnail() ) {
												...
												?>
												<div class="entry-thumb"><a href="<?php echo esc_url( $post_url ); ?>"><img src="<?php echo esc_url( $image_croped ); ?>" alt="" /></a></div>
												<?php
											}
											?>                                          
		...
		<ul class="pull-left">
			<li class="">
				<a class="kopa-sharing-item kopa-tooltip" href="http://www.facebook.com/share.php?u=<?php echo urlencode( get_permalink() ); ?>" title="Facebook" target="_blank">
					<i class="<?php echo esc_attr( $kopa['icon']['social']['facebook'] ); ?>"></i>
				</a>
			</li>
			<li class="">
				<a class="kopa-sharing-item kopa-tooltip" href="http://twitter.com/home?status=<?php echo get_the_title() . ':+' . urlencode( get_permalink() ); ?>" title="Twitter" target="_blank">
					<i class="<?php echo esc_attr( $kopa['icon']['social']['twitter'] ); ?>"></i>
				</a>
			</li>
			<li class="">
				<a class="kopa-sharing-item kopa-tooltip" href="https://plus.google.com/share?url=<?php echo urlencode( get_permalink() ); ?>" title="Google" target="_blank">
					<i class="<?php echo esc_attr( $kopa['icon']['social']['google-plus'] ); ?>"></i>
				</a></li>
			<li class="">
				<a class="kopa-sharing-item kopa-tooltip" href="http://www.linkedin.com/shareArticle?mini=true&amp;url=<?php echo urlencode( get_permalink() ); ?>&amp;title=<?php echo urlencode( get_the_title() ); ?>&amp;summary=<?php echo urlencode( get_the_excerpt() ); ?>&amp;source=<?php echo urlencode( get_bloginfo( 'name' ) ); ?>" title="Linkedin" target="_blank">
					<i class="<?php echo esc_attr( $kopa['icon']['social']['linkedin'] ); ?>"></i>
				</a>
			</li>
			<li class="">
				<a class="kopa-sharing-item kopa-tooltip" href="http://pinterest.com/pin/create/button/?url=<?php echo urlencode( get_permalink() ); ?>&amp;media=<?php echo isset( $thumbnail[0] ) ? urlencode( $thumbnail[0] ) : ''; ?>&amp;description=<?php the_title(); ?>" title="Pinterest" target="_blank">
					<i class="<?php echo esc_attr( $kopa['icon']['social']['pinterest'] ); ?>"></i>
				</a>
			</li>
			<li class="">
				<a class="kopa-sharing-item kopa-tooltip" href="mailto:?subject=<?php echo rawurlencode( $email_subject ); ?>&amp;body=<?php echo rawurlencode( $email_body ); ?>" title="Email" target="_self">
					<i class="<?php echo esc_attr( $kopa['icon']['social']['email'] ); ?>"></i>
				</a>
			</li>
		</ul>
	</div>
...
function kopa_get_about_author() {
	if ( 'true' == KopaOptions::get_option( 'is_display_post_author_information', 'true' ) ) {
		...
		<div class="about-author clearfix">
			...
			<a class="avatar-thumb" target="_blank" href="<?php echo esc_url( $link ); ?>"><?php echo get_avatar( $email, 112 ); ?></a>                
			<div class="author-content">
				<h5 itemprop="author"><a target="_blank" itemscope="itemscope" itemtype="http://schema.org/Person" class="vcard author" href="<?php echo esc_url( $link ); ?>"><span class="fn" itemprop="name"><?php echo wp_kses_post( $name ); ?></span></a></h5>
				<?php echo ($description) ? "<div>{$description}</div>" : ''; ?>
				<footer class="clearfix">
					<?php if ( $url ): ?>
						<span class="pull-left"><strong><?php _e( 'Website:', kopa_get_domain() ); ?>&nbsp;</strong><a target="_blank"  href="<?php echo esc_url( $url ); ?>"><?php echo wp_kses_post( $url ); ?></a></span>
					<?php endif; ?>
					<?php if ( 'true' == KopaOptions::get_option( 'is_display_post_author_social_links', 'true' ) ): ?>
						<ul class="pull-right social-links">
							...
							foreach ( $socials as $social ) {
								...
								if ( $tmp ) {
									?>
									<li><a target="_blank" href="<?php echo esc_url( $tmp ); ?>" class="fa <?php echo esc_attr( "fa-$social" ); ?>"></a></li>
									<?php
								}
							}
							?>                        
							<li><a target="_blank" href="<?php echo get_author_feed_link( $user_id ); ?>" class="fa fa-rss"></a></li>                        
						</ul>
					<?php endif; ?>   
					<!-- social-links -->
				</footer>    
